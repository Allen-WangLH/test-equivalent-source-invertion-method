...
if __name__ == '__main__':
    ...
    smooth_components = ['dxx','dyy','dxy','dtt']
    optimize_weights = ['obs','dxx','dyy','dxy','dtt']
    weights = {'obs':1,'refer':1,'dxx':1,'dyy':1,'dxy':1,'dtt':1}
    # weights = {'obs':1.1,'refer':1,'dxx':1.2,'dyy':1.1,'dxy':1,'dtt':1}
...
#    model.plot_field(surveys=[0,1,2,3,4,5,6],plot_station=False)
    model.plot_field(surveys=[0,1,2,3,4,5],plot_station=False)
#    model.plot_field(surveys=[0,1,2],plot_station=False)
    ...
    for i in range(6):
        filename = './output/section4.1/solution-mdtlu{}.grd'.format(i)
        print(filename,i*120,120*(i+1))
        srfgrd(model.solution[i*120:120*(i+1)], 
                              filename, nyx[0], nyx[1],xmin, xmax, ymin, ymax)

    #srfgrd(sol1, './solution-m21.grd', nyx[0], nyx[1],xmin, xmax, ymin, ymax)
    
    df1 = pd.DataFrame(columns=['lon','lat','field','inv','res','res'])
    df1['lon'] = model.orig_data['lon']
    df1['lat'] = model.orig_data['lat']
    df1['field'] = model.orig_data['g']
    df1['inv'] = fieldinv
    df1['res'] = res
    df1.to_csv('./output/section4.1/invfieldmlu.csv')    
    
    obs = [weight['obs'] for weight in model.abic_log['weights']]
    xx = [weight['dxx'] for weight in model.abic_log['weights']]
    yy = [weight['dyy'] for weight in model.abic_log['weights']]
    xy = [weight['dxy'] for weight in model.abic_log['weights']]
    tt = [weight['dtt'] for weight in model.abic_log['weights']]
    abic = model.abic_log['abic_val']

    plt.scatter(xx,abic) 
    df2 = pd.DataFrame(columns=['obs','dxx','dyy','dxy','dtt','abic_val'])
    df2['obs'] = obs
    df2['dxx'] = xx
    df2['dyy'] = yy
    df2['dxy'] = xy
    df2['dtt'] = tt
    df2['abic_val'] = abic
    df2.to_csv('./output/section4.1/abic-tc.txt') 
